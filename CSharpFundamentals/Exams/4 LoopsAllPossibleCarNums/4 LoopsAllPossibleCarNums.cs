...
namespace _4_LoopsAllPossibleCarNums
{
    class Program
    {
        static void Main(string[] args)
        {
            uint magicWeight = uint.Parse(Console.ReadLine());
            Console.WriteLine( GenerateNumber(magicWeight));
        }
        static int GenerateNumber(uint magickWeigth){
            ...
            for (int i = 0; i < letters.Length; i++)
            {
                for (int ii = 0; ii < letters.Length; ii++)
                {
                    for (int a = 0; a <= 9; a++)
                    {
                        ...
                        if(getWeight("CA" + a + a + a + a + letters[i] + letters[ii]) == magickWeigth){
                           resultCount ++;
                        }
                        for (int b = 0; b <= 9; b++)
                        {
                            if (b != a)
                            {
                                if(getWeight("CA" + a + a + a + b + letters[i] + letters[ii]) == magickWeigth){
                                   resultCount ++;
                                }
                                  if(getWeight("CA" + a + b + b + b + letters[i] + letters[ii]) == magickWeigth){
                                   resultCount ++;
                                }
                                  if(getWeight("CA" + a + a + b + b + letters[i] + letters[ii]) == magickWeigth){
                                   resultCount ++;
                                }
                                  if(getWeight("CA" + a + b + b + a + letters[i] + letters[ii]) == magickWeigth){
                                   resultCount ++;
                                }
                                if(getWeight("CA" + a + b + a + b + letters[i] + letters[ii]) == magickWeigth){
                                   resultCount ++;
                                }
                            }
                        }
        ...
        static int getWeight(string carNumber)
        {
            int resultWeight = 0;
